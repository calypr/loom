...
func main() {
	...
	fhirSchemaDir := filepath.Join(filepath.Dir(*outputDir), "fhirschema")
	if err := os.MkdirAll(fhirSchemaDir, 0755); err != nil {
		fmt.Fprintf(os.Stderr, "Error creating schema metadata directory: %v\n", err)
		os.Exit(1)
	}
	...
	if err := generateFHIRSchema(&schema, filepath.Join(fhirSchemaDir, "generated.go")); err != nil {
...
func generateTraversalCode(schema *Schema, structName string, path []string, varName string, currentType string, link Link, indent string) (string, error) {
	if len(path) == 0 {
		...
		// Calculate UUIDs and append EdgeDocuments
		forwardTargetType := targetTypeFromLabel(link.Rel)
...
func generateFHIRSchema(schema *Schema, path string) error {
	...
	for _, k := range supportedFHIRSchemaResources() {
		if isFHIRResourceDefinition(schema.Defs[k]) {
...
func supportedFHIRSchemaResources() []string {
	return []string{
		"BodyStructure",
		"Condition",
		"DocumentReference",
...
func isFHIRResourceDefinition(def *Definition) bool {
	if def == nil {
		return false
	}
	prop, ok := def.Properties["resourceType"]
	if !ok || prop == nil || prop.Const == nil {
		return false
	}
	_, ok = prop.Const.(string)
	return ok
}
