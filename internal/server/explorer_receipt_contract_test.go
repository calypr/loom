...
func TestBuilderCommandsCreateBackendOwnedDraftAndReconcileIt(t *testing.T) {
	...
	commandBody := `{"commandId":"browser-command-1","semanticsVersion":4,"snapshotToken":"` + snapshot.Token + `","expectedDraftVersion":0,"commands":[{"type":"CREATE_TABLE","title":"Patients","rootNodeId":"n_patient"}]}`
	...
	secondBody := `{"commandId":"browser-command-2","semanticsVersion":4,"snapshotToken":"` + snapshot.Token + `","expectedDraftVersion":1,"commands":[{"type":"CREATE_TABLE","title":"Visits","rootNodeId":"n_patient"}]}`
