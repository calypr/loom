...
func registerRoutes(server *api.HTTPServer, resourceService *loadapi.Service, authorizer authscope.Authorizer, graphResolver *resolver.Resolver) error {
	resourceHandler, err := loadapi.NewHandler(loadapi.Config{Service: resourceService, Authorizer: authorizer})
	if err != nil {
