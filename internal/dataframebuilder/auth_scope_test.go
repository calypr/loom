...
func TestIntrospectAndGuidedDiscoveryKeepRestrictedEmptyCatalogMode(t *testing.T) {
	resolver := dataframeBuilderRestrictedEmptyScopeResolver()
	fieldCalls := 0
	referenceCalls := 0
	service := NewService(Config{
	...
	if _, err := service.DiscoverGuided(ctx, GuidedDiscoveryRequest{Project: "P1"}); err != nil {
...
func TestPrepareRecipeKeepsRestrictedEmptyCatalogMode(t *testing.T) {
	facts := recipeServiceFacts()
	genderID := recipeServiceCapabilityID(t, facts, "Patient", "gender")
	service := NewService(Config{
		ScopeResolver: dataframeBuilderRestrictedEmptyScopeResolver(),
		DiscoverFields: func(_ context.Context, options catalog.PopulatedFieldOptions) ([]catalog.PopulatedField, error) {
			assertRestrictedEmptyFieldScope(t, options)
	...
	plan, err := service.PrepareRecipe(dataframeBuilderRestrictedEmptyContext(), RecipeRequest{Recipe: recipePreview(genderID)})
	if err != nil {
		t.Fatalf("PrepareRecipe() error = %v", err)
	}
	if plan.Builder.AuthScopeMode != authscope.ReadScopeRestricted {
		t.Fatalf("prepared recipe scope mode = %q, want restricted", plan.Builder.AuthScopeMode)
	}
	if len(plan.Builder.AuthResourcePaths) != 0 {
		t.Fatalf("prepared recipe paths = %#v, want empty", plan.Builder.AuthResourcePaths)
	}
}
