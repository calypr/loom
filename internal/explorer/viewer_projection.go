...
func projectionAliases(emitted map[string]map[string]EmittedColumn, physicalColumns map[string]map[string]publication.PhysicalColumn) map[string]map[string]string {
	aliases := map[string]map[string]string{}
	for outputID, columns := range emitted {
		for publicName := range columns {
			matches := []string{}
			for physicalName := range physicalColumns[outputID] {
				if physicalName == publicName || strings.HasSuffix(physicalName, "_"+publicName) {
					matches = append(matches, physicalName)
				}
			}
			if len(matches) == 1 {
				if aliases[outputID] == nil {
					aliases[outputID] = map[string]string{}
				}
				aliases[outputID][publicName] = matches[0]
			}
		}
	}
	return aliases
...
func emissionForPhysicalColumn(outputID, physical string, emitted map[string]map[string]EmittedColumn, aliases map[string]map[string]string) EmittedColumn {
	publicNames := make([]string, 0, len(aliases[outputID]))
	for publicName, physicalName := range aliases[outputID] {
	...
	for _, publicName := range publicNames {
		if emission := emitted[outputID][publicName]; emission.PublicColumn != "" {
			return emission
		}
	}
	return EmittedColumn{}
}

func recipeColumnLabels(bundle recipe.Bundle) map[string]map[string]string {
	labels := map[string]map[string]string{}
	var collect func(string, []recipe.Field, []recipe.Traversal)
	collect = func(output string, fields []recipe.Field, traversals []recipe.Traversal) {
		...
		for _, field := range fields {
			...
			fieldRef := strings.TrimSpace(field.FieldRef)
			if fieldRef == "" {
				fieldRef = firstExpressionSelector(field.Expr)
			}
			if fieldRef != "" {
				labels[output][field.Name] = humanizeFieldRef(fieldRef)
			}
		}
		for _, traversal := range traversals {
			collect(output, traversal.Fields, traversal.Traversals)
		}
	}
	for _, output := range bundle.Outputs {
		collect(output.Name, output.Fields, output.Traversals)
	}
	return labels
}

func firstExpressionSelector(expr recipe.Expression) string {
	if strings.TrimSpace(expr.Select) != "" {
		return strings.TrimSpace(expr.Select)
	}
	for _, argument := range expr.Args {
		if selector := firstExpressionSelector(argument); selector != "" {
			return selector
		}
	}
	return ""
...
func humanizeFieldRef(fieldRef string) string {
	value := strings.TrimSpace(fieldRef)
	if index := strings.IndexByte(value, '.'); index >= 0 {
		value = value[index+1:]
	}
	parts := strings.FieldsFunc(value, func(r rune) bool { return r == '.' || r == '_' || r == '-' || r == '[' || r == ']' })
	for index, part := range parts {
		lower := strings.ToLower(part)
		if lower == "id" || lower == "url" || lower == "uri" || lower == "uuid" {
