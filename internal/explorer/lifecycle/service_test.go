...
func (f *fakeStore) PublishAuthoring(_ context.Context, _ explorer.CompilationReceipt, revision explorer.Revision) (*explorer.Revision, error) {
	if f.order != nil {
...
func TestPreviewRejectsStaleGenerationAndScope(t *testing.T) {
	receipt := &explorer.CompilationReceipt{ID: "receipt-a", Project: "project-a", ExplorerID: "patients", SnapshotToken: "token", SourceGeneration: "generation-a", RecipeDigest: "recipe", Bundle: recipe.Bundle{Outputs: []recipe.Output{{Name: "patients"}}}}
	for _, test := range []struct {
		name     string
		...
		t.Run(test.name, func(t *testing.T) {
			...
			config.Preview = func(context.Context, recipe.Bundle, recipe.RuntimeBindings) (map[string][]map[string]any, error) {
				return nil, nil
			}
...
func TestPublishPersistsOnlyAfterActivationAndRetainsRevisionOnFailure(t *testing.T) {
	snapshot := readySnapshot("project-a", "generation-a", "token", authscope.ReadScope{Mode: authscope.ReadScopeUnrestricted})
	receipt := &explorer.CompilationReceipt{ID: "receipt-a", Project: "project-a", ExplorerID: "patients", SnapshotToken: "token", SourceGeneration: "generation-a", RecipeDigest: "recipe", CompiledConfig: []byte(`{}`), EmittedColumns: []explorer.EmittedColumn{{OutputID: "patients", PublicColumn: "patient_id"}}, Bundle: recipe.Bundle{Outputs: []recipe.Output{{Name: "patients"}}}}
	order := []string{}
	...
	config.ActivateRelease = func(context.Context, string, string, []dataset.DataframeSelector) error {
		order = append(order, "activate-release")
		return nil
	}
	config.Materialize = func(context.Context, recipe.Bundle, recipe.RuntimeBindings) (Execution, error) {
	...
	if got, want := fmt.Sprint(order), "[validate-generation materialize activate-release persist]"; got != want {
