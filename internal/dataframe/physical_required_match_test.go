...
func TestRenderPhysicalPlanRequiredInboundTraversalMatch(t *testing.T) {
	plan, err := BuildPhysicalPlan(SemanticPlan{
		...
		Root: SemanticNode{Alias: "root", ResourceType: "Patient", Children: []SemanticNode{{
			Alias: "condition", ResourceType: "Condition", EdgeLabel: "subject_Patient", MatchMode: TraversalMatchRequired,
		}}},
	})
	if err != nil {
		t.Fatal(err)
...
func TestRenderPhysicalPlanRequiredOutboundResearchStudyMatch(t *testing.T) {
	plan, err := BuildPhysicalPlan(SemanticPlan{
		...
		Root: SemanticNode{Alias: "root", ResourceType: "ResearchSubject", Children: []SemanticNode{{
			Alias: "study", ResourceType: "ResearchStudy", EdgeLabel: "study", MatchMode: TraversalMatchRequired,
		}}},
	})
	if err != nil {
		t.Fatal(err)
...
func TestBuildPhysicalPlanRequiredTraversalWithFilterUsesTypedPredicate(t *testing.T) {
	plan, err := BuildPhysicalPlan(SemanticPlan{
		...
		Root: SemanticNode{Alias: "root", ResourceType: "Patient", Children: []SemanticNode{{
			Alias: "condition", ResourceType: "Condition", EdgeLabel: "subject_Patient", MatchMode: TraversalMatchRequired,
			Filters: []TypedFilter{{FieldRef: "Condition.id", Selector: "id", FieldKind: FilterString, Operator: FilterExists}},
		}}},
	})
	if err != nil {
		t.Fatal(err)
