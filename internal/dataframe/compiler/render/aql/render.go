...
// RenderRecipeExpression renders one typed recipe physical expression without
// requiring a full legacy Builder plan. The caller owns the surrounding
// scoped root/traversal query; this helper keeps literal binding and operator
// rendering on the same path as normal physical plans.
func RenderRecipeExpression(expression PhysicalExpression, bindVars map[string]any) (string, map[string]any, error) {
	if bindVars == nil {
		bindVars = map[string]any{}
	}
	renderer := physicalPlanRenderer{bindVars: bindVars, collectionKeys: map[string]struct{}{}, setVariables: map[string]string{}, reservedVars: map[string]struct{}{}}
