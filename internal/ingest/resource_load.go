...
// loadResource preserves the unversioned, overwrite/upsert contract used by
// the project/resourceType endpoint. It deliberately does not create or
// activate a dataset generation.
func loadResource(ctx context.Context, opts LoadOptions) (LoadSummary, error) {
	...
	for _, file := range files {
		...
		result, err := loadFile(ctx, opts, client, schema, file, "", true, start, summary.VerticesInserted, summary.EdgesInserted, insertRawDocuments)
		if err != nil {
			return summary, err
		}
		...
		if err := catalogStore.WriteFieldCatalog(ctx, catalog.FieldCatalogCollection, result.Catalog.Documents(), opts.BatchSize, true, opts.WriteAPI, summary.StageSeconds); err != nil {
			return summary, err
		}
