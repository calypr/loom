...
| `traversalColumnNaming` | No | `PATH` (default) prefixes nested fields with every traversal alias; `ALIAS` uses only the globally unique owning alias; `EXACT` emits globally unique authored field names without a route prefix. |
