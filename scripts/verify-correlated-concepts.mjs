...
assert.equal(document.columns.length, 4);
const [id, systemA, systemB, cross] = document.columns.map((column) => column.column);
...
console.log(JSON.stringify({ evidence: path, assertions: 6, previewMs }, null, 2));
