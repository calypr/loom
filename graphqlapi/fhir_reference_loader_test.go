...
func TestResolveFHIRContainedReference(t *testing.T) {
	reference := "#patient"
	root := &fhirstructs.DocumentReference{
		Subject: &fhirstructs.Reference{Reference: &reference},
	}
	resolver := &Resolver{}
	...
	patient, ok := value.(*fhirstructs.Patient)
...
func TestResolveFHIRReferenceDepthLimitIsNotOptional(t *testing.T) {
	...
	ref := &fhirstructs.Reference{Reference: &reference}
	resolver := &Resolver{}
	ctx := withFHIRReferenceLoader(context.Background(), resolver)
